...
void divide_and_conquer(deque<Point>& q, int leaves = 0)
{
   ...
   if (leaves > 1) /* use multiprocessing */
   {
      int fd[2];
      ...
      if (pid > 0) /* parent process */
      {
         if (s1.size() > 1)
         {
            divide_and_conquer(s1, leaves % 2 == 0 ? leaves / 2 : (leaves / 2 + 1)); // work on the left queue  
         }

         int status;
         int waitPid = wait(&status); // wait for child process to finish
         if (status == -1)
         {
         ...
         deque<Point> s3;
         close(fd[WRITE_END]);
         recv(fd[READ_END], s3); // read left queue result from the pipe
         close(fd[READ_END]);

         merge(q, s1, s3); // merge parent process results with child process results
      ...
      else /* child process */
      {
         if (s2.size() > 1)
         {
            divide_and_conquer(s2, leaves / 2); // work on the right queue
         }

         close(fd[READ_END]);
         send(fd[WRITE_END], s2); // write right queue result to pipe
         close(fd[WRITE_END]);
   ...
   else /* do not use multiprocessing */
   {
      if (s1.size() > 1)
      {
         divide_and_conquer(s1, 0); // work on the left queue
      }
      if (s2.size() > 1)
      {
         divide_and_conquer(s2, 0); // work on the right queue
      }
